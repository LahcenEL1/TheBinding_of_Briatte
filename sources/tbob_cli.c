...
void tbob_render_engine ( WINDOW * w_main, int abs_y, int abs_x, stage_t s, int cur, int * shoot_f, int * bullet_pos_y, int * bullet_pos_x, int * bullet_pos_y_old, int * bullet_pos_x_old)
{
	if ( *shoot_f)
	{
		if ( (*bullet_pos_x_old) > 0 && (*bullet_pos_y_old) > 0 && (*bullet_pos_x_old) < D_WIDTH - 1 && (*bullet_pos_y_old) < D_HEIGHT - 1 )
			mvwprintw ( w_main, (abs_y) + ((*bullet_pos_y_old)), (abs_x) + 2 * ((*bullet_pos_x_old)), "%c", s.mtx[s.p[cur].y][s.p[cur].x]->piece[(*bullet_pos_y_old)][(*bullet_pos_x_old)]);

		
		if ( (*bullet_pos_x) > 0 && (*bullet_pos_y) > 0 && (*bullet_pos_x) < D_WIDTH - 1 &&	 (*bullet_pos_y) < D_HEIGHT - 1 )
			mvwprintw ( w_main, (abs_y) + ((*bullet_pos_y)), (abs_x) + 2 * ((*bullet_pos_x)), "*");
	}
}

void tbob_engine ( /*int abs_y, int abs_x, stage_t s, int cur, */int * shoot_f, int * bullet_pos_y, int * bullet_pos_x, int * bullet_pos_y_old, int * bullet_pos_x_old, int *dy_shoot, int * dx_shoot, double * t_ref, double * t_timer)
{
	if ( *shoot_f)
	{
		if ( (*t_timer) - (*t_ref) > 0.05)
		{
			//~ if ( (*bullet_pos_x) > 0 && (*bullet_pos_y) > 0 && (*bullet_pos_x) < D_WIDTH - 1 && (*bullet_pos_y) < D_HEIGHT - 1 )
			//~ {
				*bullet_pos_x_old = *bullet_pos_x;
				*bullet_pos_y_old = *bullet_pos_y;
			//~ }
			(*bullet_pos_x) += (*dx_shoot);
			(*bullet_pos_y) += (*dy_shoot);				
			if ( (*bullet_pos_x) < 0 || (*bullet_pos_y) < 0 || (*bullet_pos_x) > D_WIDTH - 1 || (*bullet_pos_y) > D_HEIGHT - 1 )
				(*shoot_f) = 0;
			(*t_ref) = gettime();
		}
...
void ncurses_game_loop ( WINDOW * w_main, player_t p, stage_t s)
{
	...
	int quit = 0, key = 410, var = 1;
	// Shoot
	double t_ref = gettime(), t_timer = 0.0;
	float hpMax = p.hpMax, dmg = p.dmg;
	int a_shoot = 0, dx_shoot[7] = {0}, dy_shoot[7] = {0}, shoot_f[7] = {0}, bullet_pos_x[7] = {0}, bullet_pos_y[7] = {0}, bullet_pos_x_old[7] = {0}, bullet_pos_y_old[7] = {0};
	do
	{
		t_timer = gettime();
		...
		else if ( t_act == 2 && !shoot_f[0])
		{
			dy_shoot[0] = 0;
			dx_shoot[0] = 0;
			a_shoot = tbob_get_shoot_way ( key, &(dy_shoot[0]), &(dx_shoot[0]));
			bullet_pos_x[0] = x;
			bullet_pos_y[0] = y;
			shoot_f[0] = 1;
		}
		...
		tbob_engine ( /*s, cur, */shoot_f, bullet_pos_y, bullet_pos_x, bullet_pos_y_old, bullet_pos_x_old, dy_shoot, dx_shoot, &t_ref, &t_timer);

		for (int i = 0; i < 7; i++)
		{
			tbob_render_engine ( w_main, abs_y, abs_x, s, cur, shoot_f+i, bullet_pos_y+i, bullet_pos_x+i, bullet_pos_y_old+i, bullet_pos_x_old+i);
		}
		// Display engine function after updates
